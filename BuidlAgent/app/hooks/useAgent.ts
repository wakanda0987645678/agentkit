...
async function messageAgent(userMessage: string): Promise<string | null> {
  try {
    console.debug("[useAgent] Sending message to /api/agent:", userMessage);
    ...
    if (!response.ok) {
      console.error("[useAgent] /api/agent returned non-OK status:", response.status);
    }

    const data = (await response.json()) as AgentResponse;
    console.debug("[useAgent] Received response from /api/agent:", data);
